...
const syncSequelize = async () => {
    await exports.Memes.sync({ force: settings_1.DB_MIGRATE });
    await exports.WordCounts.sync({ force: settings_1.DB_MIGRATE });
    if (settings_1.DB_SEED) {
        exports.WordCounts.create({
            word: settings_1.RICO_TRIGGER,
            count: 33
        });
        memes_json_1.memeCommands.forEach(({ name, message }) => exports.Memes.create({
            name,
            message
        }));
    }
